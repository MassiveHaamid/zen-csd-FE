...
const Dashboard = () => {
  ...
  const [attendanceChartData, setAttendanceChartData] = useState({
    ...
    datasets: [
      {
        data: [loggedUser.presentDays, loggedUser.totalDays - loggedUser.presentDays],
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await fetch('https://caps-be.onrender.com/api/attendance');
        const data = await response.json();
  ...
  return (
      ...
      <div className="score text-center">{webCode ? webCode.score : "Not Submitted"}</div>
      <div className='text-center mb-2'>
      <Link to="/webcode" className="view_btn">View</Link>
      </div>
      </div>
      <div className="wkata">
      <div className="head">Capstone-1 Score</div>
      <div className="score text-center">{capStone ? capStone.score : "Not Submitted"}</div>
      <div className='text-center mb-2'>
      <Link to="/capstone" className="view_btn">View</Link>
      </div>
      </div>
      <div className="wkata">
      ...
      <div className='text-center mb-2'>
      <Link to="/mock" className="view_btn">View</Link>
      </div>
      </div>
      </div>
      </div>
